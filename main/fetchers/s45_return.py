"""S 45 trains from Wien Heiligenstadt toward Hütteldorf (through Hernals).

Publishes the next N departures on commute/return/leg3/s45/heiligenstadt/departures.
"""
import json
import logging
import threading
...
TOPIC = "commute/return/leg3/s45/heiligenstadt/departures"
POLL_INTERVAL_S = 30
LOOKAHEAD_MIN = 90
...
def is_return_s45(dep: dict) -> bool:
    line = dep.get("line") or ""
    direction = dep.get("direction") or ""
    return line.startswith("S 45") and "Hütteldorf" in direction
...
def run_once(mqtt_client) -> None:
    ...
    filtered = [d for d in all_deps if is_return_s45(d)]
    ...
    log.info(f"published {len(filtered)} S 45 return departures (mid={info.mid})")
